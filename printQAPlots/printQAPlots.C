...
//Changing this code in order to run on files with JetID and no JetID cut. - Owen Baron, Dec. 12 2016

// env and I/O ------------------------
const std::string CMSSW_BASE="/net/hisrv0001/home/obaron/CMSSW_7_5_8/";//CMSSW_BASE
const std::string SCRATCH_BASE="/export/d00/scratch/ilaflott/5p02TeV_ppJetAnalysis/readForests/11.22.16_outputCondor/";//10.20.16_outputCondor/";
const std::string outputDir=CMSSW_BASE+"src/5p02TeV_ppJetAnalysis/printQAPlots/output/";//output dir
...
int printQAPlots(const std::string input_ppData_condorDir , const std::string input_ppMC_condorDir , 
  ...
  std::string theJetIDratioPDFFileName=outputDir+fullJetType;;
  theJetIDratioPDFFileName+="_"+output_PDFname_base+"JetIDratios.pdf";
  ...
  open_thePDFFileName=thePDFFileName+"[";  close_thePDFFileName=thePDFFileName+"]";
   ...
   std::string open_theJetIDPDFFileName, close_theJetIDPDFFileName;
  open_theJetIDPDFFileName=theJetIDratioPDFFileName+"[";  close_theJetIDPDFFileName=theJetIDratioPDFFileName+"]";
  ...
  //else if(doJetIDPlots)std::cout<<std::endl<<"doing JetID plots, skipping evt QA plots..."<<std::endl<<std::endl;
	...
	temp_canvEvt->Print( theJetIDratioPDFFileName.c_str() ); 
	...
	temp_canvEvt_wLog->Print( theJetIDratioPDFFileName.c_str() );	
      }
	...
	// open hists
	std::string inHistName="hWeightedVz";
	std::cout<<" drawing JetId/no JetID Ratio hist "<<inHistName<<std::endl<<std::endl;	
	TH1F* theDataEvtQAHist= (TH1F*)finData->Get( inHistName.c_str() );
  ...
  std::string jetIDInt;
  if(doJetIDPlots)jetIDInt="1"; //include cut
  else jetIDInt="0";

  
  // jet plots----------------------
  if(!drawJetQAPlots)std::cout<<std::endl<<"skipping jet QA plots..."<<std::endl<<std::endl; //here's the plots made that I want right now
  else { 
	...
	if(debugMode)std::cout<<std::endl<<"creating temporary canvas for printing JetID ratio Jet plots..."<<std::endl;
	TCanvas *temp_canvJetCutRat = new TCanvas("tempJetId", "tempJetId", 600, 600);
    temp_canvJetCutRat->cd();
    temp_canvJetCutRat->SetLogy(0);
    ...
    for(int j=0;j<N_vars;j++){ std::cout<<std::endl;
	  ...
	  std::string inHistName="hJetQA_"+jetIDInt+"wJetID_"+var[j]; //normal
	   
      std::string inHistNamecut="hJetQA_1wJetID_"+var[j];      //for jetID cuts
      std::string inHistNamenocut="hJetQA_0wJetID_"+var[j];      
	...
	->TH1::Rebin(var_xAx_reBin[j]);	
	  ...
	  //Set up the JetID cut plots
	  
	  if(debugMode)std::cout<<" opening input Data w/ cut"<<inHistNamecut<<std::endl<<std::endl;      
      TH1F* theDataJetcutQAHist= (TH1F*)
	( (TH1*)finData->Get(inHistNamecut.c_str()) )
	->TH1::Rebin(var_xAx_reBin[j]);	
      ...
      theDataJetcutQAHist->Scale( 1./4.);//eta bin width scaling
      theDataJetcutQAHist->SetMarkerStyle(kCircle);
      theDataJetcutQAHist->SetMarkerSize(0.99);
      theDataJetcutQAHist->SetMarkerColor( theDataOverlayMarkerColor );
      theDataJetcutQAHist->SetLineColor( theDataOverlayLineColor );
...
//DRAW JETID CUT HISTOS
	...
	theCutRatio->SetAxisRange(0,2.,"Y");
...
/*
	...
	*/
	...
	std::cout<<"Printing Jet-ID Cut Histos"<<std::endl;
	temp_canvJetCutRat->Print( theJetIDratioPDFFileName.c_str() );
	//temp_canvJetRat->Print("JetIDRatio.pdf");
  ...
  if(!drawJetTrigQAPlots)
	...
	//Nevermind lads, we only need this for a trigger ratio plot!
	/*
	if(debugMode)std::cout<<std::endl<<"creating temporary canvas for printing JetID/noJetID plots..."<<std::endl;
	TCanvas *temp_canvJetCutRatio = new TCanvas("tempJetCutRatio", "tempJetCutRatio", 1200, 600);
    temp_canvJetCutRatio->cd();
    temp_canvJetCutRatio->SetLogy(0);
    */
	...
	if(j==0){
	  ...
	  //DRAW CANVAS FOR JET ID CUT RATIO
	  //Oh wait nevermind; do I want this thing at all??
	/*
	inHistName+="cut_"+radius+etaWidth;	
	TH1F* theCutRatio=(TH1F*)
	  ( (TH1*)finData->Get(inHistName.c_str()) )
	  ->TH1::Rebin(jetTrigQABinning/2);
	
	theCutRatio->SetMarkerStyle(theTrigOverlayMarker[j]);
	theCutRatio->SetMarkerSize(0.99);
	theCutRatio->SetMarkerColor( theTrigOverlayMarkerColor[j] );
	theCutRatio->SetLineColor( theTrigOverlayLineColor[j] );	

	JetTrigRatioLegend->AddEntry(theCutRatio,(HLTName_Leg[j]+"/TrigComb.").c_str(),"lp");

	TH1F* comboHistClone=(TH1F*)theComboHist->Clone("comboHistClone4Div");
       	theCutRatio->Divide(comboHistClone);
